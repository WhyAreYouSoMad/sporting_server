...
public interface StadiumRepository extends JpaRepository<Stadium, Long>, StadiumCustomRepository {
        ...
        // AdminStadium - 승인대기
        @Query("SELECT new shop.mtcoding.sporting_server.admin.stadium.dto.AdminWaitStadiumListOutDto(st.id, st.name, st.status, ft.fileUrl, st.createdAt) "
                        + "FROM Stadium st "
                        + "INNER JOIN st.category ct "
                        + "INNER JOIN ProfileFile ft on ft.fileInfo.id = st.fileInfo.id "
        ...
        // AdminStadium - 비활성화
        @Query("SELECT new shop.mtcoding.sporting_server.admin.stadium.dto.AdminInactiveStadiumListOutDto(st.id, st.name, st.status, ft.fileUrl, st.createdAt) "
                        + "FROM Stadium st "
                        + "INNER JOIN st.category ct "
                        + "INNER JOIN ProfileFile ft on ft.fileInfo.id = st.fileInfo.id "
                        + "WHERE st.status != '운영중' and st.status != '승인대기' "
                        + "ORDER BY CASE WHEN st.status = '비활성' THEN 0 ELSE 1 END")
        Page<AdminInactiveStadiumListOutDto> getInactiveStadiumList(Pageable pageable);

        // AdminStadium - 비활성화 검색
        @Query("SELECT new shop.mtcoding.sporting_server.admin.stadium.dto.AdminInactiveStadiumListOutDto(st.id, st.name, st.status, ft.fileUrl, st.createdAt) "
                        + "FROM Stadium st "
                        + "INNER JOIN st.category ct "
                        + "INNER JOIN ProfileFile ft on ft.fileInfo.id = st.fileInfo.id "
                        + "WHERE st.status != '운영중' and st.status != '승인대기' and st.name LIKE %:name%")
        Page<AdminInactiveStadiumListOutDto> getInactiveStadiumListByName(Pageable pageable,
        ...
        @Query("SELECT new shop.mtcoding.sporting_server.topic.stadium.dto.StadiumListOutDTO(st.id, ct.sport, st.name, sct.courtPrice, ft.id, ft.fileUrl) "
                        ...
                        + "WHERE ct.sport = :sportKeyword and st.status = '운영중' "
                        + "GROUP BY st.id")
